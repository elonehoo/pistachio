...
describe('localStorage', () => {
  ...
  describe('sync behavior', () => {
    ...
    let clearSpy: any = undefined as any
    let removeItemSpy: any = undefined as any
    let updateItemSpy: any = undefined as any

    beforeEach(() => {
      useWebStorage('localStorage').remove()
      storage = useWebStorage('localStorage').store

      clearSpy = vi.spyOn(storage, 'clear')
      removeItemSpy = vi.spyOn(storage, 'removeItem')
      updateItemSpy = vi.spyOn(storage, 'updateItem')
    })
    ...
    it('should show quota errors', async () => {
      function getStorageTotalSize(upperLimit?: /* in bytes */ number) {
        ...
        const testkey = '$_test' // (NOTE: Test key is part of the storage!!! It should also be an even number of characters)
        const test = function (_size: number) {
          try {
        ...
        let low = 0
        let high = 1
        const _upperLimit = (upperLimit || 1024 * 1024 * 1024) / 2
        let upperTest = true
        ...
        if (high > _upperLimit)
          high = _upperLimit
        store.removeItem(testkey)
        for (const p in backup) store.setItem(p, backup[p])
