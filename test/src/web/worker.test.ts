...
import { exposeWorker, useWorker } from '@elonehoo/pistachio'
...
describe('worker', () => {
  ...
  describe('exposeWorker', () => {
    ...
    it('should post messages', async () => {
      const warnSpy = vi.spyOn(console, 'warn')
      const func = vi.fn().mockImplementation(x => x)
      ...
      expected = _gen()
      await send(expected)
      const generated = Array.from(_gen())
      ...
      postMessageFn.mockClear()
      const _genAsync = async function*() {
