import { usePromise,promisedTimeout } from '@elonehoo/pistachio'
...
describe("promise", () => {
  ...
  it("should not assign error if not the last promise", async () => {
    ...
    use.exec().catch((x:any) => expect(x).toMatch("Error"));
    use.exec();
  ...
  it("should only update result after the promise is resolved", async () => {
    ...
    let v = 1;
    const use = usePromise((x:any) => promisedTimeout(20).then(() => (v = x)));
