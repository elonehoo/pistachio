import { beforeAll, describe, expect, it, vi, afterAll,afterEach } from 'vitest'
import { useRetry, exponentialDelay, promisedTimeout } from '@elonehoo/pistachio'
import {nextTick} from '../utils'

describe("retry", () => {
  const fnFactory = vi.fn();
  ...
  it("should cancel the retry if other task is executed", async () => {
    ...
    expect(isRetrying.value).toBe(false);
    expect(nextRetry.value).toBe(undefined);
  ...
  it("should not retry forever", () => {
    const { exec } = useRetry(vi.fn(() => Promise.reject("err")));
  ...
  it("should cancel retry after call cancel()", async () => {
    ...
    expect(isRetrying.value).toBe(false);
    expect(retryErrors.value).toHaveLength(0);
    expect(nextRetry.value).toBeUndefined();
    ...
    expect(isRetrying.value).toBe(true);
    expect(retryErrors.value[1]).toBe(undefined);
