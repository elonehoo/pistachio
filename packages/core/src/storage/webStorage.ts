...
import { isClient, isString, unwrap, wrap } from '../utils'
...
let storageMap: Map<WebStorageType, WebStorage> | undefined
  ...
  if (supported && storage) {
    if (!store) {
      ...
      store = {
        ...
        getRef<T>(k: RefTyped<string>, debounce = true) {
          const item = ref<T>()

          const keyWatch = watch(
            wrap(k),
            (k) => {
              const data = storage.getItem(k)
              if (!data)
          ...
          const valueWatch = watch(
            item,
            debounce ? useDebounce(updateFn, ms) : updateFn,
